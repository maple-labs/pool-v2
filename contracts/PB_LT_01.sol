...
contract PB_LT_01 {
        ...
        )
    {
        ...
        if (state.expectedAt == 0) {
            ...
            state.issuanceRate = state.expectedInterest * POOL.precision() / state.paymentInterval;
        ...
        if (claimedAssets_ == state.closingPrincipal + state.closingInterest) {
            ...
            uint256 falseInterest = state.issuanceRate * timeElapsed / POOL.precision();
        ...
        if (block.timestamp <= state.expectedAt) {
            ...
            adjustment   += int256(state.issuanceRate * (state.expectedAt - block.timestamp) / POOL.precision());
        ...
        // Assuming a late payment was made.
            ...
            if (block.timestamp > POOL.vestingPeriodFinish()) {
                // Ignore the time only if the vesting period finish was not reduced due to another IM which uses shortest timestamps.
                if (state.expectedAt <= POOL.vestingPeriodFinish()) {
                    extraTime -= block.timestamp - POOL.vestingPeriodFinish();
                }
            }

            // Calculate the amount that was already vested by accident.
            uint256 alreadyVested = state.issuanceRate * extraTime / POOL.precision();
    ...
    {
        principalOut_        = POOL.principalOut() - principalReturned_;
        freeAssets_          = uint256(int256(POOL.freeAssets()) + adjustment_);
        issuanceRate_        = uint256(int256(POOL.issuanceRate()) + acceleration_);
        vestingPeriodFinish_ = POOL.vestingPeriodFinish();
